...
class ODTW:
    ...
    def run(sample_name, chunk=1024, format=pyaudio.paInt16, rate=44100, channels=2):
        
        wf = wave.open(sample_name, 'rb')

        outp = pyaudio.PyAudio()
        # open stream (2)
        sample = outp.open(format=outp.get_format_from_width(wf.getsampwidth()),
        ...
        X_byte = stream.read(chunk)
        Y_byte = wf.readframes(chunk)
        X_arr, Y_arr = np.frombuffer(X_byte), np.frombuffer(Y_byte)
